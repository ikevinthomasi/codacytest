...
public partial class ED_User_Close_COI_Details : System.Web.UI.Page
{
    DataRow row = null;
    DataView dv = null;
    String qs = "";
    ...
    protected void btnClose_Click(object sender, EventArgs e)
    {
        ...
        qs = qs + " SET ";
        qs = qs + " hod_close_resolution = @phod_close_resolution, ";
        qs = qs + " hod_close_url = @phod_close_url, ";
        qs = qs + " status = @pstatus, ";
        qs = qs + " close_date = @pclose_date, ";
        qs = qs + " close_by = @pclose_by ";
        ...
        cmd.Parameters.AddWithValue("@pdeclare_no", Request.QueryString["rid"].ToString());
        cmd.Parameters.AddWithValue("@pstaff_ic", Request.QueryString["rpid"].ToString());
        cmd.Parameters.AddWithValue("@phod_close_resolution", fldHODRemarks.Text);
        cmd.Parameters.AddWithValue("@phod_close_url", urlStr);
    ...
    private void emailnotification(String reqNo, String coiDesc)
    {
        ...
        bodyMail = bodyMail + "</tr>";
        bodyMail = bodyMail + "<tr>";
        bodyMail = bodyMail + "<td colspan='3'>The following COI has been closed in the port:<br/><br/></td>";
        bodyMail = bodyMail + "</tr>";
        bodyMail = bodyMail + "<tr>";
        ...
        bodyMail = bodyMail + "</tr>";
        bodyMail = bodyMail + "<tr>";
        bodyMail = bodyMail + "<td>HOD Completion Justification</td>";
        bodyMail = bodyMail + "<td>:</td>";
        bodyMail = bodyMail + "<td>" + fldHODRemarks.Text + "</td>";
        bodyMail = bodyMail + "</tr>";
        bodyMail = bodyMail + "<tr>";
        ...
        qs = qs + " WHERE           email_address IS NOT NULL AND status = '1' AND staff_ic = '" + fldICNo.Text + "' ";
        if (con.State == System.Data.ConnectionState.Closed)
            con.Open();
        cmd = new SqlCommand(qs, con);
        cmd.CommandTimeout = 0;
        ...
        if (dt.Rows.Count != 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                row = null;
                row = dt.Rows[i];
                mailMsg.To.Add(row["email_address"].ToString());
            }
        }

        mailMsg.From = new MailAddress("eDeclaration@uemedgenta.uemnet.com");
        mailMsg.Subject = "COI CLOSED [FOR INFO ONLY] : " + reqNo;
    ...
    private String uploadAttachment(String reqId, String ICNo)
    {
        ...
        String filenameStr = "HOD_Close_" + reqId;
